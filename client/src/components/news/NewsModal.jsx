...
function HighlightBlock({ title, content, variant = 'primary' }) {
  const styles =
    variant === 'primary'
      ...
      : 'bg-app border-app';
  const titleColor = variant === 'primary' ? 'text-primary' : 'text-muted';

  return (
    <div className={`rounded-control border p-4 ${styles}`}>
      <p className={`text-xs font-bold uppercase tracking-wide mb-2 ${titleColor}`}>{title}</p>
      <p className="text-sm text-app leading-relaxed">{content || '—'}</p>
    </div>
  );
}
      ...
      <div className="flex flex-wrap gap-1.5">
        {items.map((item, i) => {
          ...
          return (
            <span
              key={`${name}-${i}`}
              className="badge-pill bg-primary/10 text-primary"
            >
              {name}
...
export default function NewsModal({ article, isOpen, onClose }) {
  ...
  const hasImage = Boolean(getArticleImage(article));
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={article.title} wide>
      <div className="space-y-5">
        {hasImage && (
          <ArticleMedia
            article={article}
            className="aspect-[16/9] w-full rounded-control"
            alt=""
          />
        )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          ...
          {article.channel && (
            <div>
              ...
              <p className="text-sm font-medium text-app">{article.channel}</p>
            </div>
          )}
          <div>
          ...
          <div>
            <p className="text-xs text-muted mb-1">Subcategory</p>
            <p className="text-sm font-medium text-app">{safeString(article.subcategory)}</p>
          </div>
          <div>
          ...
          {article.isActionRequired && (
            <div className="col-span-2">
              <p className="text-xs text-muted mb-1">Department Responsible</p>
              <p className="text-sm font-medium text-app">{article.department}</p>
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <p className="text-sm text-app">{formatLocation(article.location)}</p>
          {article.location?.state && (
            <p className="text-xs text-muted mt-0.5">{article.location.state}</p>
          )}
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-app">
          ...
          {article.source_url && (
            <a
              ...
              className="inline-flex items-center gap-1.5 rounded-md bg-primary px-4 py-2 text-sm font-medium text-white hover:bg-primary-hover transition-colors"
            >
