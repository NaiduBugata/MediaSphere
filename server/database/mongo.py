...
def build_postid_map(collector_json_path: Path | str) -> dict[str, dict[str, Any]]:
    ...
    Returns:
        Mapping of normalized title -> {post_id, url, created_on, thumbnail, raw_title}.
    """
    ...
    for article in articles:
        ...
        if not thumbnail:
            ...
            if isinstance(raw, dict):
                # Prefer fields already extracted by the collector when present;
                # fall back to first Lokal images[] entry for older collector JSON.
...
def _resolve_post_id(
    ...
    Returns:
        Tuple of (post_id, source_url, created_on, thumbnail).
    """
    normalized = _normalize_title(title)
    ...
    if title in title_map:
        ...
        return f"hash_{digest}", None, None, None
...
def upsert_articles(
    ...
    for article in news_articles:
        title = article.get("title", "")
        post_id, source_url, created_on, thumbnail = _resolve_post_id(title, title_map)
...
def _resolve_youtube_post_id(
    ...
    if title in title_map:
        ...
        if meta is None:
            ...
            return f"{YOUTUBE_POST_ID_PREFIX}hash_{digest}", None, None, None, None
...
def upsert_youtube_articles(
    ...
    for article in news_articles:
        title = article.get("title", "")
        post_id, source_url, created_on, channel, thumbnail = _resolve_youtube_post_id(title, title_map)
        if not thumbnail and source_url:
            # Fallback derived from watch URL / post_id when collector lacked thumbs.
            vid = str(post_id).removeprefix(YOUTUBE_POST_ID_PREFIX) if str(post_id).startswith(YOUTUBE_POST_ID_PREFIX) else ""
            if vid and not vid.startswith("hash_"):
