...
def _first_http_url(*candidates: Any) -> str:
    """Return the first non-empty http(s) URL string from candidates."""
...
def extract_lokal_thumbnail(post: Dict[str, Any]) -> str:
    """
    Derive a thumbnail URL from a raw Lokal API post.

    Preference order matches live Lokal payloads (`images[].image` /
    `images[].thumb_url`) plus common alternate keys if present.
    """
...
def normalize_article(post: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    return {
        ...
        "thumbnail": extract_lokal_thumbnail(post),
        "raw": post,
    }
